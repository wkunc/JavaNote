...
## SimpleAliasRegistry
这个类实现了上面的接口 
首先 alias 不可以一样, 即一个 alias 不能对应两个对象
...
public class SimpleAliasRegistry implements AliasRegistry {
    ...
    // 所以不可以有相同的别名, 但是同一个 name 可以有多个别名
		...
		synchronized (this.aliasMap) {
			if (alias.equals(name)) {
				...
				if (registeredName != null) {
                    ...
                    // 如果 name 和 registeredName 不同,就判断是否允许别名覆盖
    ...
    // 查看 alias 链(就是递归调用),确定 name 是否已经注册了给定 alias
    // 相当重要的方法
...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
    ...
    // key 是 beanName, value 是对应的 bean 包含的 属性名
	private final Map<String, Set<String>> containedBeanMap = new ConcurrentHashMap<String, Set<String>>(16);

    // key 是 beanName, value 是对应的 bean 依赖的 bean 的name
	private final Map<String, Set<String>> dependentBeanMap = new ConcurrentHashMap<String, Set<String>>(64);

    // key 是 beanName, value 是对应的 bean 依赖的 bean
