...
>
> Notice that this might allow SQL injection when using plain statements and the provided input is not properly
> sanitized. Also notice that for prepared statements, if the stream length is not specified when using '
> PreparedStatement.set*Stream()', the driver would not be able to determine the optimum number of parameters per batch
> and might return an error saying that the resultant packet is too large.
